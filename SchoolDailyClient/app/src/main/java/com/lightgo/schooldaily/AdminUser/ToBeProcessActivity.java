...
public class ToBeProcessActivity extends Activity implements Handler.Callback{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        httpThread = new HttpThread("ad/ActivityShow",0);
        httpThread.start();
    ...
    private void SliptToBeProcess(String str){
        try {
            ...
            if(infos == null){
                infos = new Info[array.length()];
            }
            for (int i = 0; i < array.length(); i++){
                infos[i] = new Info();
               infos[i].mTitle= array.getJSONObject(i).getString("activityTitle");
                infos[i].mPlace ="活动地点："+array.getJSONObject(i).getString("activityPlace");
                infos[i].mTime = "开始时间："+array.getJSONObject(i).getString("activityTime");
                if(array.getJSONObject(i).getString("activityState").equals(1)){
                    infos[i].mIsAudit= "活动状态：正在进行中";
                }else infos[i].mIsAudit="活动状态：还未开始";

                infos[i].mIsEnd= "结束时间:"+array.getJSONObject(i).getString("activityData");
            }
            if(simpleAdapter == null){
                simpleAdapter = new SimpleAdapter(
                        this,
                        GetData(),
                        R.layout.activity_to_be_processex,
                        new String[]{"mTitle","mPlace","mTime","mIsAudit","mIsEnd"},
                        new int[]{R.id.ActivityTitle,R.id.ActivityPlace,R.id.ActivityTime,R.id.IsAudit,R.id.IsEnd});
