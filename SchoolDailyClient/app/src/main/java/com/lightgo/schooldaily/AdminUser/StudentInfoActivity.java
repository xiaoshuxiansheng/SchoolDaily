...
public class StudentInfoActivity extends Activity implements View.OnClickListener,Handler.Callback,AdapterView.OnItemClickListener{
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        HttpThread httpThread = new HttpThread("ad/AndroidStudentDetailsInfo",1);
        httpThread.start();
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.SelectFinish:
                ...
                HttpThread httpThread = new HttpThread("ad/AndroidClassStudentInfo",0);
                httpThread.start();
                break;
            case R.id.myw:
            case R.id.myl:
    ...
    private void SliptClassStudentInfo(String str){
        ...
        try {
            ...
            infos = new Info[Array.length()];//先声明此结构体大小，没有为此分配内存
            for (int i = 0; i < Array.length(); i++){
                infos[i]= new Info();//再声明此内容为
                infos[i].mStudentID=Array.getJSONObject(i).getString("studentId");
                infos[i].mStudentName = Array.getJSONObject(i).getString("studentName");
                infos[i].mStudentKPI = Array.getJSONObject(i).getString("studentKpl");
                infos[i].mStudentTel = Array.getJSONObject(i).getString("studentPhone");
            }
            if(simpleAdapter == null){
                simpleAdapter = new SimpleAdapter(
                        this,
                        GetData(),
                        R.layout.activity_student_infoex,
                        new String[]{"mStudentID","mStudentName","mStudentKPI","mStudentTel"},
                        new int[]{R.id.mStudentID,R.id.mStudenetName,R.id.mStudentKPI,R.id.mStudentTel});
    ...
    class HttpThread extends Thread implements Runnable {
        private String pathAPI = "http://192.168.98.1:8080/";
        ...
        public void run() {
            ll = new JSONObject();
            try {
                if(mAction  == "ad/AndroidStudentDetailsInfo"){
