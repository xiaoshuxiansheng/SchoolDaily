...
public class TodayCheckActivity extends Activity implements View.OnClickListener,Handler.Callback {
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.bt_zzx:
                ...
                HttpThread MhttpThread = new HttpThread("ad/AndroidMorCheckInfo",0);
                MhttpThread.start();
                break;
            case R.id.bt_wzx:
                btnDrawable = resources.getDrawable(R.drawable.buttonstyle);
    ...
    private void SliptDormitoryInfo(String str){
        try {
            ...
            infoLists = new Info[array.length()];
            for (int i = 0; i < array.length(); i++){
                infoLists[i] = new Info();
                infoLists[i].mGrade= array.getJSONObject(i).getString("checkGrade");
                infoLists[i].mMajor = array.getJSONObject(i).getString("checkMajor");
                infoLists[i].mClassName= array.getJSONObject(i).getString("className");
                infoLists[i].StudentName= array.getJSONObject(i).getString("studentName");
                infoLists[i].AttendanceTime= array.getJSONObject(i).getString("checkTime");
            }
        ...
        try {
            ...
            if(simpleAdapter == null){
                simpleAdapter = new SimpleAdapter(
                        this,GetData(),
                        R.layout.activity_today_checkex,
                        new String[]{"mGrade","mMajor","mClassName","StudentName","AttendanceTime"},
                        new int[]{R.id.mjjj,R.id.mzyy,R.id.mbjj,R.id.mxmm,R.id.msj});
                mToDayCheck_ListView.setAdapter(simpleAdapter);
            }else {
                lists.clear();
                GetData();
