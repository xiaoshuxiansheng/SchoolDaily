...
public class HCheckActivity extends Activity implements View.OnClickListener,AdapterView.OnItemClickListener,Handler.Callback{
    ...
    private String pathAPI = "http://192.168.98.1:8080/ad/";
    ...
    private void load() {
        ...
        Thread thread1 = new Thread() {
            ...
            public void run() {
                try {
                    String s = httpUrlConnection(pathAPI + "DormitoryCheck", ll.toString());//toString();把JSONArray对象转换为json格式的字符串
    ...
    void GetHostelData(String str){
        try {
            ...
            info = new HostelInfo[array.length()];
            for (int i=0;i<array.length();i++)
            {
                info[i] = new HostelInfo();
                info[i].mStuID=array.getJSONObject(i).getString("studentId");
                info[i].mStuName=array.getJSONObject(i).getString("studentName");

                if(array.getJSONObject(i).getString("studentClass").equals("一班")){
                    info[i].mHostelName="01寝室";
                }else info[i].mHostelName="02寝室";
                info[i].Isflag = false;
            }
        } catch (JSONException e) {
            e.printStackTrace();
        ...
        mClassAllCount.setText("班级总人数："+info.length);
    }
